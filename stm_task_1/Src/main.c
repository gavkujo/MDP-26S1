...
int main(void)
{
  ...
  OLED_Init();
  OLED_Clear();
  OLED_ShowString(0, 0, (uint8_t*)"MDP Robot Init...");
 ...
 //Encoder_Init();
  ...
  // Initialize IMU
    ...
    HAL_Delay(600);
    IMU_CalibrateGyroZ(&hi2c2, 0, GYRO_FULL_SCALE_250DPS);
  // Initialize IR Sensors (stub)
  // IR_Init();

  OLED_ShowString(0, 32, (uint8_t*)"Init Complete");
  OLED_Clear();
  HAL_Delay(2000);
...
void test_icm20948(void) {
    uint8_t who = 0;
    if (inv_io_hal_read(0x00, &who, 1) == 0) {  // WHO_AM_I reg = 0x00
        if (who == 0xEA) {
            printf("ICM-20948 present: WHO_AM_I=0x%02X\r\n", who);
        } else {
            printf("Device responded but WHO_AM_I=0x%02X (expected 0xEA)\r\n", who);
        }
    } else {
        printf("No response from ICM-20948. Check wiring/I2C address/pull-ups!\r\n");
    }
}
/* USER CODE END 4 */
